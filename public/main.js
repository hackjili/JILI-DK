...
// Lucky / Normal key clock
const luckyLines=["🔥 ভাগ্যের দরজা খোলা! বড় বাজি ধরুন 💰✨","🌟 আজকের দিন Spin করুন 🥳🎉","💎 সাহসী হোন, ধনী হতে পারেন 😍"];
const normalLines=["⏳ শান্ত থাকুন, ছোট বাজি যথেষ্ট 🙂","🌼 ধীরে চলুন, অভিজ্ঞতা অর্জন করুন 🌸"];
...
function updateClock(){
  const now=new Date(); const min=now.getMinutes(); const sec=now.getSeconds();
  ...
  const lastDigitMin=min%10; const displayTime=`${lastDigitMin}:${String(sec).padStart(2,"0")}`;

  if(isLucky){
    clock.textContent=generateLuckyKey();
    tinyTime.textContent=displayTime;
    timerCard.style.background="#f472b6";
    ...
    tinyTime.textContent=displayTime;
    timerCard.style.background="#9ca3af";
